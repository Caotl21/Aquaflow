...
OUT = os.path.abspath('src/aquaflow_stonefish/data/bricsbot/bricsbot_stonefish_physical_proxy_1000.obj')
...
target_dims = (0.38242, 0.277425, 0.358082)
cur_dims = tuple(obj.dimensions)
center = sum((v.co for v in obj.data.vertices), start=__import__('mathutils').Vector()) / len(obj.data.vertices)
scale = [target_dims[i] / cur_dims[i] for i in range(3)]
for v in obj.data.vertices:
    v.co = center + __import__('mathutils').Vector((v.co.x-center.x, v.co.y-center.y, v.co.z-center.z)) * __import__('mathutils').Vector(scale)
obj.data.update()
